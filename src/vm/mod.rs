...
impl Vm {
            ...
            if self.has_pending_failure {
                while let Some(frame) = self.frames.last_mut() {
                    ...
                    if let Some(handler) = handler {
                        ...
                        self.stack
                            .truncate(frame.stack_base + handler.stack_depth as usize);
                        break;
                    }
                    self.frames.pop();
                }
    ...
    fn box_local(&mut self, index: usize) -> ObjectId {
        let value = std::mem::take(&mut self.locals[index]);
        let obj_id = self.heap.alloc_obj(value);
        self.locals[index] = Value::Ref(obj_id);
        obj_id
    }
    ...
    fn exec_push_char32(&mut self) {
        ...
        self.stack.push(Value::Char32(ch));
    }
    ...
    fn exec_push_string(&mut self) {
        ...
        self.stack.push(Value::Ref(obj_id));
    }

    fn exec_push_logic(&mut self, value: bool) {
        self.stack.push(Value::Logic(value));
    }
    ...
    fn exec_push_type(&mut self) {
        ...
        self.stack.push(Value::Type(TypeId(type_id)));
    }
    ...
    fn exec_store_local(&mut self) {
        let value = self.stack.last().unwrap().copy_value();
    ...
    fn exec_load_upvalue(&mut self) {
        ...
        let value = match *upvalue.borrow() {
            Upvalue::Open { stack_index } => self.locals[stack_index].clone(),
    ...
    fn exec_make_tuple(&mut self) {
        ...
        let start = self.stack.len() - elem_cnt as usize;
    ...
    fn exec_set_tuple_element(&mut self) {
        ...
        let elem = self.stack.last().unwrap().copy_value();
        match tuple {
            ...
            .map(|upvalue| match upvalue {
                UpvalueDesc::EnclosingLocal { depth, slot } => {
                    ...
                    let upvalue = Upvalue::Open {
                        stack_index: frame.local_base + slot.0,
                    };
                    let index = frame
                        .captures
                        .iter()
    ...
    fn exec_make_object(&mut self) {
        ...
        let methods = self.stack.split_off(self.stack.len() - method_count);
    ...
    fn exec_load_object_field(&mut self) {
        ...
        match obj {
            Value::Ref(obj_id) => match self.heap.fetch_obj(obj_id) {
                Value::Object { fields, .. } => self.stack.push(fields[field_index].clone()),
                _ => unreachable!(),
            },
            _ => unreachable!(),
        }
    ...
    fn exec_unwrap(&mut self) {
        let inner = match self.stack.pop().unwrap() {
